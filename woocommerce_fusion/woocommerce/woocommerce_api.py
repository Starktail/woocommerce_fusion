...
class WooCommerceResource(Document):
            ...
            WooCommerceAPI(
                api=APIWithRequestLogging(
                    ...
                    timeout=(
                        10,
                        40,
                    ),  # (connect_timeout, read_timeout) - prevents SSL handshake hangs
                ...
                while True:
                    ...
                    if response.status_code == 400:
                        try:
                            ...
                            if error_data.get(
                                "code"
                            ) == "rest_invalid_param" and "status" in error_data.get(
                                "data", {}
                            ).get(
                                "params", {}
                            ):
    ...
    def db_insert(self, *args, **kwargs):
        ...
        try:
            ...
            error_type = (
                "Connection timeout" if isinstance(timeout_err, ConnectTimeout) else "Read timeout"
            )
            log_and_raise_error(
                exception=timeout_err,
    ...
    def db_update(self, *args, **kwargs):
        ...
        keys_to_pop = [
            ...
            if record_before_save.get(key) == value
            or str(record_before_save.get(key)) == str(value)
        ]
