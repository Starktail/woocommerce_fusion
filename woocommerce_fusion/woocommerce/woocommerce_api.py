...
class WooCommerceResource(Document):
    ...
    def _init_api() -> List[WooCommerceAPI]:
        ...
        wc_servers = [frappe.get_doc(
            "WooCommerce Server", server.name) for server in wc_servers]
        ...
        if len(wc_api_list) == 0:
            frappe.throw(
                _("At least one WooCommerce Server should be Enabled"), SyncDisabledError)
    ...
    def load_from_db(self):
        ...
        wc_server_domain, record_id = get_domain_and_id_from_woocommerce_record_name(
            self.name)
        ...
        try:
            record = self.current_wc_api.api.get(
                f"{self.resource}/{record_id}").json()
    ...
    def get_list_of_records(cls, args) -> list[Union[Dict, "WooCommerceResource"]]:
        ...
        if len(wc_api_list) > 0:
            ...
            params["per_page"] = min(
                per_page + offset, wc_records_per_page_limit)

            # Map Frappe filters to WooCommerce parameters
            if "filters" in args and args["filters"]:
                updated_params = get_wc_parameters_from_filters(
                    args["filters"])
            ...
            for wc_server in wc_api_list:
                ...
                if response.status_code != 200:
                    log_and_raise_error(
                        error_text="get_list failed", response=response)
                ...
                if "x-wp-total" in response.headers:
                    count_of_total_records_in_api = int(
                    ...
                    try:
                        ...
                        log_and_raise_error(
                            error_text="Unexpected response", response=response)
                ...
                while True:
                    ...
                    end = min(len(results), per_page -
                              len(all_results) + start)
                    ...
                    params["offset"] = current_offset
                    try:
                        response = wc_server.api.get(
                            cls.resource, params=params)
                    except Exception as err:
                        log_and_raise_error(err, error_text="get_list failed")
                    if response.status_code != 200:
                        log_and_raise_error(
    ...
    def get_count_of_records(cls, args) -> int:
        ...
        for wc_server in wc_api_list:
            ...
            if response.status_code != 200:
                log_and_raise_error(
                    error_text="get_count failed", response=response)
    ...
    def db_insert(self, *args, **kwargs):
        ...
        # Select the relevant WooCommerce server
        self.current_wc_api = next(
            (api for api in self.wc_api_list if self.woocommerce_server ==
             api.woocommerce_server),
            None,
        )
        ...
        if response.status_code != 201:
            log_and_raise_error(
                error_text="db_insert failed", response=response)
    ...
    def db_update(self, *args, **kwargs):
        ...
        record_before_save = self.deserialize_attributes_of_type_dict_or_list(
            record_data_before_save)
        ...
        wc_server_domain, id = get_domain_and_id_from_woocommerce_record_name(
            self.name)
        ...
        if response.status_code != 200:
            log_and_raise_error(
                error_text="db_update failed", response=response)
    ...
    def to_dict(self):
        ...
        """
        doc_dict = {field.fieldname: self.get(
            field.fieldname) for field in self.meta.fields}
