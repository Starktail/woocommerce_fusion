...
class TestWooCommerceOrder(FrappeTestCase):
    ...
    def test_get_list_returns_orders_with_name_attribute(self, mock_init_api):
        ...
        for order in orders:
            ...
            expected_name = (
                urlparse(woocommerce_server_url).netloc + WC_ORDER_DELIMITER + str(order.id)
            )
    ...
    def test_get_list_pagination_works(self, mock_init_api):
        ...
        test_parameters = [
            ...
            frappe._dict(
                {"args": {"page_length": 10, "start": 10}, "expected_order_counts": (0, 10, 0)}
            ),
            frappe._dict(
                {"args": {"page_length": 10, "start": 30}, "expected_order_counts": (0, 0, 10)}
            ),
            frappe._dict(
                {"args": {"page_length": 20, "start": 5}, "expected_order_counts": (5, 15, 0)}
            ),
            frappe._dict(
                {"args": {"page_length": 5, "start": 40}, "expected_order_counts": (0, 0, 5)}
            ),
            frappe._dict(
                {"args": {"page_length": 60, "start": 0}, "expected_order_counts": (10, 20, 30)}
            ),
        ]
        ...
        with patch.object(WooCommerceOrder, "__init__", return_value=None) as mock_init:
            ...
            with patch.object(WooCommerceOrder, "call_super_init") as mocked_super_call:
                ...
                with patch.object(
                    ...
                    woocommerce_order.name = (
                        woocommerce_server + WC_ORDER_DELIMITER + str(order_id)
                    )
    ...
    def test_db_insert_makes_post_call(self, mock_init_api):
        ...
        self.assertEqual(
            mock_api_list[0].api.post.call_args.kwargs["data"]["status"], "Hello World"
        )
    ...
    def test_db_update_makes_put_call(self, mock_update_shipment_tracking, mock_init_api):
        ...
        self.assertEqual(
            mock_api_list[0].api.put.call_args.kwargs["data"]["status"], "Hello World"
        )
