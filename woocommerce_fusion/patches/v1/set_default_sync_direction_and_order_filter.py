...
def execute():
    ...
    try:
        ...
        for wc_server in wc_servers:
            ...
            if (
                not wc_server_doc.order_status_filter
                or len(wc_server_doc.order_status_filter) == 0
            ):
                # Add default statuses: processing, completed
                for status in ["processing", "completed"]:
                    wc_server_doc.append(
                        "order_status_filter", {"woocommerce_order_status": status}
                    )
        ...
        print(
            _("Failed to set default sync direction and order status filter on WooCommerce Server")
        )
