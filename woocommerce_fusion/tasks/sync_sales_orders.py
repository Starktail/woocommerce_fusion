...
def run_sales_order_sync_from_hook(doc, method):
    if (
        ...
        frappe.enqueue(run_sales_order_sync, queue="long",
...
def run_sales_order_sync(
    ...
    if woocommerce_order or woocommerce_order_name:
        ...
        if not sales_order.woocommerce_server:
            frappe.throw(_("No WooCommerce Server defined for Sales Order {0}").format(
                sales_order_name))
...
def sync_woocommerce_orders_modified_since(date_time_from=None):
    ...
    wc_orders += get_list_of_wc_orders(
        date_time_from=date_time_from, status="trash")
...
class SynchroniseSalesOrder(SynchroniseWooCommerce):
    ...
    def get_corresponding_sales_order_or_woocommerce_order(self):
        ...
        if self.sales_order and not self.woocommerce_order and self.sales_order.woocommerce_id:
            ...
            wc_server = frappe.get_cached_doc(
                "WooCommerce Server", self.sales_order.woocommerce_server)
    ...
    def get_erpnext_sales_order(self):
        ...
        filters.append(["Sales Order", "woocommerce_id",
        ...
        if len(sales_orders) > 0:
            self.sales_order = frappe.get_doc(
                "Sales Order", sales_orders[0].name)
            ...
            if (
                ...
                if get_datetime(self.woocommerce_order.woocommerce_date_modified) > get_datetime(
                    ...
                    self.update_sales_order(
                        self.woocommerce_order, self.sales_order)
                if get_datetime(self.woocommerce_order.woocommerce_date_modified) < get_datetime(
                        self.sales_order.modified
                ):
    ...
    def create_and_link_payment_entry(
        ...
        if not wc_server:
            raise ValueError(
                "Could not find woocommerce_server in list of servers")
        ...
        if (
            ...
            payment_method_bank_account_mapping = json.loads(
                wc_server.payment_method_bank_account_mapping)
            ...
            if company_bank_account:
                ...
                payment_method_gl_account_mapping = json.loads(
                    wc_server.payment_method_gl_account_mapping)
                company_gl_account = payment_method_gl_account_mapping[wc_order.payment_method]

                # Create a new Payment Entry
                company = frappe.get_value(
                    "Account", company_gl_account, "company")
                ...
                if not payment_reference_no:
                    payment_reference_no = (
                        next(
                            (data["value"] for data in meta_data if data["key"]
                             == "yoco_order_payment_id"),
                            None,
                        )
                ...
                for i, so_item in enumerate(sales_order.items):
                    if not so_item.woocommerce_id:
                        break
                            ...
                            int(
                                so_item.woocommerce_id) != line_items[i]["product_id"]
                ...
                replacement_line_items.extend(
                    [
                        {"product_id": so_item.woocommerce_id,
                            "quantity": so_item.qty, "price": so_item.rate}
                        for so_item in sales_order.items
                    ]
                )
    ...
    def create_sales_order(self, wc_order: WooCommerceOrder) -> None:
        ...
        self.create_missing_items(wc_order, json.loads(
            wc_order.line_items), wc_order.woocommerce_server)
    ...
    def create_missing_items(self, wc_order, items_list, woocommerce_site):
        ...
        for item_data in items_list:
            item_woo_com_id = cstr(item_data.get(
                "variation_id") or item_data.get("product_id"))
    ...
    def set_items_in_sales_order(self, new_sales_order, wc_order):
        ...
        for item in json.loads(wc_order.line_items):
            woocomm_item_id = item.get(
                "variation_id") or item.get("product_id")
            ...
            if woocomm_item_id == 0:
                ...
                found_item = frappe.get_doc(
                    "Item", item_codes[0].parent) if item_codes else None
            ...
            if not wc_server.use_actual_tax_type:
                ...
                add_tax_details(new_sales_order, ordered_items_tax,
                                "Ordered Item tax", wc_server.tax_account)
        ...
        if not new_sales_order.shipping_rule:
            add_tax_details(new_sales_order, wc_order.shipping_tax,
                            "Shipping Tax", wc_server.f_n_f_account)
            add_tax_details(
                new_sales_order,
    ...
    def create_address(
        ...
        address.country = frappe.get_value(
            "Country", {"code": raw_data.get("country", "IN").lower()})
...
def get_list_of_wc_orders(
    ...
    if minimum_creation_date:
        filters.append(["WooCommerce Order", "date_created",
                       ">", minimum_creation_date])
    if sales_order:
        filters.append(["WooCommerce Order", "id", "=",
                       sales_order.woocommerce_id])
    if status:
    ...
    while new_results:
        ...
        new_results = woocommerce_order.get_list(
            args={"filters": filters, "page_lenth": page_length,
                  "start": start, "as_doc": True}
        )
