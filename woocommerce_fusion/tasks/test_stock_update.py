...
class TestWooCommerceStockSync(FrappeTestCase):
    ...
    def test_update_stock_levels_on_woocommerce_site(self, mock_wc_api, mock_frappe):
        ...
        actual_put_endpoints = [
            call.kwargs["endpoint"] for call in mock_api_instance.put.call_args_list
        ]
    ...
    def test_update_stock_levels_on_woocommerce_site_variant(self, mock_wc_api, mock_frappe):
        ...
        mock_frappe.get_doc.side_effect = [
            ...
            frappe._dict(
                woocommerce_servers=[
                    frappe._dict(
                        woocommerce_id=100, woocommerce_server="woo1.example.com", enabled=1
                    ),
                ]
            ),
        ]
    ...
    def test_update_stock_levels_for_all_enabled_items_in_background(
        ...
        expected_calls = [
            call(
                doctype="Item", filters={"disabled": 0}, fields=["name"], start=0, page_length=500
            ),
            call(
                doctype="Item",
                filters={"disabled": 0},
