frappe.ui.form.on("Item", {
  refresh: function (frm) {
    ...
    // Add a custom button to sync Item Family (only for template items)
    if (frm.doc.has_variants) {
      frm.add_custom_button(
        __("Sync this Item Family with WooCommerce"),
        function () {
          frm.trigger("sync_item_family");
        },
        __("Actions")
      );
    }
  ...
  sync_item_family: function (frm) {
    // Sync this Item Family (template and all variants)
    frappe.dom.freeze(__("Syncing Item Family with WooCommerce..."));
    frappe.call({
      method: "woocommerce_fusion.tasks.sync_items.sync_item_family",
      args: {
      ...
      callback: function (r) {
        ...
        frappe.show_alert(
          {
            message: __("Item family sync completed successfully"),
            indicator: "green",
          },
          5
        );
