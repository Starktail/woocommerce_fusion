    ...
    def autoname(self):
        ...
        if self.woocommerce_id and self.woocommerce_server:
            wc_server = frappe.get_cached_doc(
                "WooCommerce Server", self.woocommerce_server)
            if wc_server.sales_order_series:
                self.name = make_autoname(key=wc_server.sales_order_series)
            else:
                # Get idx of site
                wc_servers = frappe.get_all(
                    "WooCommerce Server", fields=["name", "creation"])
                sorted_list = sorted(
                    wc_servers, key=lambda server: server.creation)
                idx = next(
                    (index for (index, d) in enumerate(sorted_list)
                     if d["name"] == self.woocommerce_server), None
                )
            ...
            if wc_server.enable_so_status_sync:
                ...
                if mapping:
                    if self.woocommerce_status != mapping.woocommerce_sales_order_status:
                        ...
                        frappe.enqueue(run_sales_order_sync,
                                       queue="long", sales_order_name=self.name)
...
def get_woocommerce_order_shipment_trackings(doc):
    ...
    if doc.woocommerce_server and doc.woocommerce_id:
        wc_order = get_woocommerce_order(
            doc.woocommerce_server, doc.woocommerce_id)
...
def get_woocommerce_order(woocommerce_server, woocommerce_id):
    ...
    wc_order = frappe.get_doc(
        {"doctype": "WooCommerce Order", "name": wc_order_name})
